<template>
	<div class="modify">
		...
		<div class="article" :class="isEdit && 'isEdit'">
			...
			<!-- 新增联户表井 -->
			<template v-if="Number(prtype) === columns[3].value">
				<WaterMeter ref="waterMeterEle" :data="data" :onHandleCancel="onHandleBack" @onHandleSubmit="onHandleSubmit"></WaterMeter>
			</template>
			<!-- 新增蓄水池 -->
			...
			onHandleCreateAndUpdate(params) {
				...
				switch (Number(prtype)) {
					...
					case 4:
						...
						break;
					case 5:
						that.$refs.waterMeterEle._isSaveSuccess(false);
						/**
						 * 检查联户表井编码是否存在
						...
						dseWrLhbjBCtrlCheckExist(
								...
								.then((_ = {}) => {
									...
									return saveOrUpDseWrLhbjB(params, that).then((results = {}) => {
										...
										if (status) {
											that.$refs.waterMeterEle._isSaveSuccess(true);
